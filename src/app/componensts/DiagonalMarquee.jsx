    ...
    <div className="relative w-full h-[25vh] lg:h-[50vh] overflow-hidden flex items-center justify-center bg-gray-100">
      <motion.div
       ...
       onMouseLeave={() => setBox({ ...box, isHover: false })}
        className={`absolute flex space-x-16 h-[10vh] lg:h-[20vh] items-center whitespace-nowrap ${deg == 0 ? "rotate-[-5deg]" :"rotate-[6deg]"} rounded-full bg-black`} // Rotate text diagonally
  ...
  >
    {
      Array.from({length:2}).map((_,idx)=>(
        ...
        {offers.map((item, index) => (
          <span key={index} className="text-2xl lg:text-4xl text-white font-bold px-8 active:bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text active:text-transparent  transition duration-100">
            {item}
          </span>
        ))}
